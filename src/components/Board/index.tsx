...
function Display({ value, onClick }: DisplayProps) {
  ...
  if (value < Date.now()) {
    return (
      <button
        type="button"
        onClick={onClick}
        className="font-mono text-white px-4 py-2 rounded-3xl bg-stone-400"
      >
        --:--
      </button>
    );
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="font-mono text-white px-4 py-2 rounded-3xl bg-lime-600"
    >
      {format(value)}
    </button>
  );
...
export function Timer() {
  ...
  useInterval(() => {
    ...
    // It must be active, greater than 0 (meaning it has been set) and in the past.
    // Since we updated the state above, next time it ticks `active` will be false.
